...
vim.lsp.config('esbonio', {
    ...
    local function esbonio_scroll_view()
      local client =
        vim.lsp.get_clients({ name = 'esbonio', bufnr = 0 })[1]
      if not client then
        return
      end
    ...
    local function esbonio_preview_file()
      ...
      for _, client in ipairs(
        vim.lsp.get_clients({ name = 'esbonio', bufnr = 0 })
      ) do
...
local esbonio_preview_group =
  vim.api.nvim_create_augroup('EsbonioAutoPreview', { clear = true })
